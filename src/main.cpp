...
#if defined(NDEBUG)
# error "Insuracoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x3edf6d315d6b0c8b569e90efd74728e0184f3dc0f287d2d53ef82b084af2b010");
...
const string strMessageMagic = "Insuracoin Signed Message:\n";
...
{
    // Insuracoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 525960); // Insuracoin: About every year roughly

    return nSubsidy + nFees;
}

//The NScrypt Injection
//Beginning injection of Scrypt-Adaptive-N Code
// yacoin: increasing Nfactor gradually
const unsigned char minNfactor = 10;
const unsigned char maxNfactor = 30;
int64 nChainStartTime = 1496082636;
...
unsigned int GetNextWorkRequired(const CBlockIndex *pindexLast, const CBlockHeader *pblock)
{
    ...
    int64_t genesisBlockNTime = 1496082636;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Insuracoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x459445b5b610ccfc424967723acab1cd5107b9a572847dd1a27092672ea64aa7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis block
        const char* pszTimestamp = "CNBC 29/May/17 Bitcoin correction sees nearly $4 billion wiped off value";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("047d4dadaa7c75db4a26d719dd5cc7a6d03b3263b2dbb575a76b2ee83a6504d936129340b6d0c8c25fa3854cadb5e87fa0a41249f5cde86fdbd190ba3f224b538e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1496082636;						//Next addition, change from testnet settings
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3362572;

        if (fTestNet)
        {
            block.nTime    = 1496075787;
            block.nNonce   = 2121030;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xc66fbe218c13331d93325a8e5f5f9cc7174d0d17c40593dfa8536f5f937b158e"));
		//NScrypt Injection
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc2, 0xb8, 0xdd }; // Insuracoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("InsuracoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("InsuracoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static InsuracoinMiner(CWallet *pwallet)
{
    printf("InsuracoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("insuracoin-miner");
    ...
    try { loop {
        ...
        printf("Running InsuracoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("InsuracoinMiner terminated\n");
        throw;
    }
}
...
void static ThreadAltcoinMiner(void* parg)
{
    CWallet* pwallet = (CWallet*)parg;
    try
    {
        vnThreadsRunning[THREAD_MINER]++;
        AltcoinMiner(pwallet);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadAltcoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadAltcoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadAltcoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&InsuracoinMiner, pwallet));
}
